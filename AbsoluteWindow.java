...
public class AbsoluteWindow extends JPanel implements ActionListener,MouseListener,MouseMotionListener{
	...
	AbsoluteWindow(int no_routers, int no_lans) {
		routers = no_routers;
		lans = no_lans;
		a = new int[routers];
		...
		panel.add(done);
		image2 = new ImageIcon("/home/dell/eclipse/eclipse_workspace/ppl/refresh1.png");
		...
		image = new ImageIcon("/home/dell/eclipse/eclipse_workspace/ppl/router1.png");
		image1 = new ImageIcon("/home/dell/eclipse/eclipse_workspace/lan1.png");
		...
		for(int i = 0;i<2;i++) {
			...
			if(i == 0)
				lan_icon_button[0].setToolTipText("Sender Lan");
			if(i == 1)
				lan_icon_button[1].setToolTipText("Receiver Lan");
	...
	public void actionPerformed(ActionEvent e) {
			if(e.getSource() == done) {
				...
				if(doneCount == 1){
					...
					new Graph(matrix, row0, rowlast, 10,parents);
					int index, counter = 0;
					index = rowlast;
				...
				if(counter == 1) {
					...
					label1.setText("");
					new FinalWindow(routingTable, path, routers);
					...
					new Graph(matrix, row0, rowlast, routers, parents);
				...
				new InfoWindow(path,router_degree,  routers);
			}
